...
class WPCommon {
  ...
  async setup() {
    ...
    const { cache, resolve, experiments, output, stats, externals, target } =
      this;
    // init config
  ...
  get target(): Configuration['target'] {
    return store.config.build.wpTarget
      ? store.config.build.wpTarget
      : ['web', store.config.build.target];
  }
  get externals(): Configuration['externals'] {
    return store.biuShare.externals;
  }
  ...
  get resolve(): Configuration['resolve'] {
    ...
    const rs = {
      modules: [
        ...
        store.biuResolve('node_modules'), // emp
    ...
    if (configResolve.modules) {
      rs.modules =
        configResolve.extends === false
          ? configResolve.modules
          : [...rs.modules, ...configResolve.modules];
    }
    if (configResolve.alias) {
      rs.alias =
        configResolve.extends === false
          ? configResolve.alias
          : { ...rs.alias, ...configResolve.alias };
    }
    if (configResolve.extensions) {
      rs.extensions =
        configResolve.extends === false
          ? configResolve.extensions
          : [...rs.extensions, ...configResolve.extensions];
    }
