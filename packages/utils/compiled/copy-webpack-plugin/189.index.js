...
// EXTERNAL MODULE: external "@fe6/biu-utils/compiled/fast-glob"
var fast_glob_ = __webpack_require__(325);
...
;// CONCATENATED MODULE: ../../node_modules/.pnpm/registry.npmmirror.com+globby@12.2.0/node_modules/globby/gitignore.js
...
const isGitIgnored = async options => {
	...
	const paths = await fast_glob_('**/.gitignore', options);
...
const isGitIgnoredSync = options => {
	...
	const paths = fast_glob_.sync('**/.gitignore', options);
...
;// CONCATENATED MODULE: ../../node_modules/.pnpm/registry.npmmirror.com+globby@12.2.0/node_modules/globby/index.js
...
const globby = async (patterns, options) => {
	...
	const paths = await Promise.all(tasks.map(task => fast_glob_(task.pattern, task.options)));
...
const globbySync = (patterns, options) => {
	...
	for (const task of tasks) {
		matches = array_union(matches, fast_glob_.sync(task.pattern, task.options));
	}
...
const globbyStream = (patterns, options) => {
	...
	return merge2(tasks.map(task => fast_glob_.stream(task.pattern, task.options)))
		.pipe(filterStream)
		.pipe(uniqueStream);
};
...
const isDynamicPattern = (patterns, options = {}) => {
	...
	return [patterns].flat().some(pattern => fast_glob_.isDynamicPattern(pattern, options));
};
